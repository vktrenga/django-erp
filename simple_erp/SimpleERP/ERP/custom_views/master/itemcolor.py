...
def itemcolor_create(request):
    if request.method == 'GET':
        return Response({'data':'', 'module':'Item Color'}, template_name='ERP/master/itemcolor/create_update.html')
    else:
        serializer = ItemColorSerializer(data=request.data)
        if serializer.is_valid():
            user_id= session_user_id(request)
            date_modified=store_date_time()
            serializer.save(created_by=user_id,created_date=date_modified)
            if request.accepted_renderer.format == 'html':
                return Response({"success_data": "Data added successfully"}, template_name='ERP/master/itemcolor/create_update.html')
            ...
            if request.accepted_renderer.format == 'html':
                return Response({"error_data": data}, template_name='ERP/master/itemcolor/create_update.html')
...
def itemcolor_view(request, id):
    data_obj = ItemColor.objects.get(id=id)
    if request.method == "GET":
        ser_data = ItemColorSerializer(data_obj).data
        if request.accepted_renderer.format == 'html':
            return Response({"data": ser_data,"view_mode":1}, template_name='ERP/master/itemcolor/create_update.html')
        return Response({"data": ser_data,"view_mode":1}, status=status.HTTP_200_OK)

@api_view(['GET', 'POST','Delete'])
#@permission_classes((IsAuthenticated, ))
def itemcolor_update(request, id):
    data_obj = ItemColor.objects.get(id=id)
    ser_data = ItemColorSerializer(data_obj).data
    if request.method == "GET":
        if request.accepted_renderer.format == 'html':
            return Response({"data": ser_data}, template_name='ERP/master/itemcolor/create_update.html')
        return Response({"data": ser_data}, status=status.HTTP_200_OK)
    else:
        serializer = ItemColorSerializer(data_obj, data=request.data, partial=True)
        if serializer.is_valid():
            user_id= session_user_id(request)
            date_modified=store_date_time()
            ...
            if request.accepted_renderer.format == 'html':
                return HttpResponseRedirect(reverse('ERP:itemcolor_list'))
            ...
            if request.accepted_renderer.format == 'html':
                return Response({"error_data": data,"data": ser_data}, template_name='ERP/master/itemcolor/create_update.html') 
            return Response(data, status=status.HTTP_400_BAD_REQUEST)



def itemcolor_delete(request,id):
    selected_values=ItemColor.objects.get(pk=id)
    user_id= session_user_id(request)
    date_modified=store_date_time()
...
def itemcolor_list(request):
     ...
     try:
        ...
        pass
     model_obj = ItemColor.objects.filter(**custom_filter)
     model_data = ItemColorSerializer(model_obj, many=True).data
     ...
     if request.accepted_renderer.format == 'html':
        return Response({"data": model_data,'module':'Item Color',"custom_filter":custom_filter}, template_name='ERP/master/itemcolor/list.html')
