...
def tax_create(request):
    if request.method == 'GET':
        return Response({'data':'', 'module':'Tax'}, template_name='ERP/master/tax/create_update.html')
    else:
        ...
        if serializer.is_valid():
            ...
            if request.accepted_renderer.format == 'html':
                return Response({"success_data": "Data added successfully"}, template_name='ERP/master/tax/create_update.html')
            ...
            if request.accepted_renderer.format == 'html':
                return Response({"error_data": data}, template_name='ERP/master/tax/create_update.html')
...
def tax_view(request, id):
    data_obj = Tax.objects.get(id=id)
    if request.method == "GET":
        ser_data = TaxSerializer(data_obj).data
        if request.accepted_renderer.format == 'html':
            return Response({"data": ser_data,"view_mode":1}, template_name='ERP/master/tax/create_update.html')
...
def tax_update(request, id):
    ...
    if request.method == "GET":
        ser_data = TaxSerializer(data_obj).data
        if request.accepted_renderer.format == 'html':
            return Response({"data": ser_data}, template_name='ERP/master/tax/create_update.html')
        return Response({"data": ser_data}, status=status.HTTP_200_OK)
    else:
        serializer = TaxSerializer(data_obj, data=request.data, partial=True)
        if serializer.is_valid():
            user_id= session_user_id(request)
            date_modified=store_date_time()
            ...
            if request.accepted_renderer.format == 'html':
                return HttpResponseRedirect(reverse('ERP:tax_list'))
...
def tax_delete(request,id):
    selected_values=Tax.objects.get(pk=id)
    user_id= session_user_id(request)
    date_modified=store_date_time()
...
def tax_list(request):
     ...
     try:
        if request.data['tax_name']:
            custom_filter['tax_name']=request.data['tax_name']
        if request.data['tax_group']:
            custom_filter['tax_group']=request.data['tax_group']
     except:
        pass
     model_obj = Tax.objects.filter(**custom_filter)
     model_data = TaxSerializer(model_obj, many=True).data
     ...
     if request.accepted_renderer.format == 'html':
        return Response({"data": model_data,'module':'Tax',"custom_filter":custom_filter}, template_name='ERP/master/tax/list.html')
